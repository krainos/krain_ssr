...
@krainos 
1
8 3 luvvien/ssr-install-shellscript
 Code  Issues 1  Pull requests 0  Projects 0  Wiki  Security  Insights
ssr-install-shellscript/ssr.sh
 Vien domain
c9ce5fa on 8 Jun
1533 lines (1513 sloc)  60.3 KB
...
#=================================================
#	System Required: CentOS 6+/Debian 6+/Ubuntu 14.04+/CentOS 7+
#	Description: Install the ShadowsocksR server
#	Version: 2.0.59
#	Author: krain
#	Blog: https://www.csfo.cn
#=================================================
...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://github.com/krainos/krain_ssr/blob/master/bbr.sh; then
...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="https://www.csfo.cn/qrcode?base64=${SSurl}"
...
ssr_link_qr(){
	...
	SSRQRcode="https://www.csfo.cn/qrcode?base64=${SSRurl}"
...
接下来客户端下载和配置请参考：https://www.csfo.cn/"
		echo && echo "==================================================="
	else
...
接下来客户端下载和配置请参考：https://www.csfo.cn"
	...
	echo -e "vultr限时注册送50刀：https://www.csfo.cn"
	echo -e "outline搭建vpn教程：https://www.csfo.cn"
	echo -e "更多文章：https://www.csfo.cn"
	echo -e "进入 https://www.csfo.cn！"
	echo "==================================================="
}
# 设置 配置信息
Set_config_port(){
	while true
	do
	echo -e "请输入要设置的ShadowsocksR账号 端口,最近严查，尽量小一点的端口"
...
Set_config_password(){
	...
	read -e -p "(默认: www.csfo.cn):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="www.csfo.cn"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://github.com/krainos/krain_ssr/blob/master/service/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://github.com/krainos/krain_ssr/blob/master/service/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Update_SSR(){
	SSR_installation_status
	echo -e "因暂停更新ShadowsocksR服务端，所以此功能临时禁用。"
	#cd ${ssr_folder}
	#git pull
	#Restart_SSR
}
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://github.com/krainos/krain_ssr/blob/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://github.com/krainos/krain_ssr/blob/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://github.com/krainos/krain_ssr/blob/master/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	wget -N --no-check-certificate "https://github.com/krainos/krain_ssr/blob/master/ssr.sh" && chmod +x ssr.sh
